class MessageManager:
	...
	@classmethod
	async def add_reaction_and_event(cls, message, emoji, user_id, handler,
	                                 *args):
		if isinstance(user_id, tuple):
			for user in user_id:
				cls.reaction_events[(message.id, emoji, user)] = (handler,
				                                                  args)
		else:
	...
	async def on_raw_reaction(cls, payload):
		e, n, b = payload.emoji, payload.emoji.name, "<"
		if e.id:
			if e.animated:
				b += "a"
			n = b + ":" + n + f":{e.id}>"
		container = (payload.message_id, n, payload.user_id)
