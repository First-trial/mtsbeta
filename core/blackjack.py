...
from core.singlep import SinglePlayerGame, WON, LOST, QUIT, DRAW
from core.em import STOP, ALPHABET, SPLIT
from core._blackjack import Blackjack
...
class BlackjackDiscord(SinglePlayerGame):
	...
	async def start_game(self):
		...
		await MessageManager.add_reaction_and_event(self.message,
		                                            ALPHABET["s"],
		                                            self.player.id,
		                                            self.on_stand_reaction)

		if self.blackjack.can_split():
			await MessageManager.add_reaction_and_event(
			    self.message, SPLIT, self.player.id, self.on_split_reaction)
	...
	async def on_hit_reaction(self):
		...
		await MessageManager.edit_message(self.message, self.get_board())
		await MessageManager.remove_reaction(self.message, ALPHABET["h"],
		                                     self.player.member)
