...
class Card:
	...
	def __str__(self):
		return _suit[self.suit] + " " + str(_vals[self.rank])
		...
		for suit in suits:
			for rank in ranks:
				self.deck.append(Card(
				    suit, rank))  # appending the Card object to self.deck list

	def __str__(self):
		composition = ''  # set the deck comp as an empty string
		for card in self.deck:
			composition += '\n' + card.__str__(
			)  # Card class string representation
...
class Blackjack:
	...
	def can_split(self):
		return self.player_hands[0].cards[0].rank == self.player_hands[
		    0].cards[1].rank
	...
	def is_dealer_busted(self):
		...
		)[0] > 21 and self.dealer_hand.get_value()[1] > 21

	def dealer_turn(self):
		self.player_turn = False
		while self.dealer_hand.get_value(
		)[0] < 17 or self.dealer_hand.get_value()[1] < 17:
			self.dealer_hand.add_card(self.deck.deal())
