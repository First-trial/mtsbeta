...
from core.em import STOP, REPEAT
...
class SinglePlayerSession(Session):
	...
	async def pause(self):
		...
		await MessageManager.edit_message(
		    self.message,
		    self.game.get_board() + "\n" + self.get_summary())

		await MessageManager.add_reaction_and_event(self.message, STOP,
		                                            self.player.id, self.close)
		await MessageManager.add_reaction_and_event(self.message, REPEAT,
		                                            self.player.id,
		                                            self.start_game)
	...
	def get_summary(self):
		lst = [[
		    "Player", "Wins", "Losses", "Draws", "Unfinished", "Total played"
		],
		       [
		           self.player.name, self.player.wins, self.player.losses,
		           self.player.draws, self.player.unfinished,
