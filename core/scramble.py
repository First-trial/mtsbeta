from core.singlep import SinglePlayerGame, WON, LOST, QUIT
from core.msgmanager import MessageManager
from core.em import ALPHABET, STOP, ARROW_LEFT
from core._scramble import Scramble
...
class ScrambleDiscord(SinglePlayerGame):
	...
	async def start_game(self):
		...
		await MessageManager.add_reaction_and_event(self.message, STOP,
		                                            self.player.id,
		                                            self.on_quit_game)
		await MessageManager.add_reaction_and_event(self.message, ARROW_LEFT,
		                                            self.player.id,
		                                            self.on_back_reaction)
		for c in self.scramble_game.scrambled_word:
			await MessageManager.add_reaction_and_event(
			    self.message, ALPHABET[c], self.player.id,
			    self.on_letter_reaction, ALPHABET[c])
		...
		for char, emoji in ALPHABET.items():
			if emoji == letter_emoji:
				...
				if char not in self.scramble_game.scrambled_word:
					await MessageManager.clear_reaction(
					    self.message, letter_emoji)
				else:
					await MessageManager.remove_reaction(
					    self.message, letter_emoji, self.player.member)
				break
		await MessageManager.edit_message(self.message, self.get_board())
	...
	async def on_back_reaction(self):
		...
		if char != "_":
			await MessageManager.add_reaction_and_event(
			    self.message, ALPHABET[char], self.player.id,
			    self.on_letter_reaction, ALPHABET[char])
		await MessageManager.remove_reaction(self.message, ARROW_LEFT,
		                                     self.player.member)
		await MessageManager.edit_message(self.message, self.get_board())
	...
	def get_board(self):
		...
		elif len(self.scramble_game.scrambled_word
		         ) == 0 and not self.scramble_game.has_won():
