...
from core.generic import Database
from core.generic import create_table
...
class DatabaseManager:
	...
	def on_startup(cls, bot):
		...
		cls.database.create_table("players", [
		    "player_id integer", "minigame text", "time_stamp integer",
		    "wins integer", "losses integer", "draws integer",
		    "total_games integer", "time_played integer", "unfinished integer"
		], ["player_id", "minigame", "time_stamp"])
		cls.database.create_table("minigames", [
		    "server_id integer", "minigame text", "time_stamp integer",
		    "wins integer", "losses integer", "draws integer", "total_games",
		    "time_played integer", "unfinished integer"
		], ["server_id", "minigame", "time_stamp"])

		cls.database.create_table(
		    "servers",
		    ["server_id integer", "time_stamp integer", "event text"], [
		        "server_id",
		        "time_stamp",
		    ])
	...
	def get_weekly_stats_for_player_of_month(cls, player_id, date_):
		...
		stats = dict()
		for week in weeks:
			stats[week.strftime("%W")] = cls.get_stats_for_player_of_week(
			    player_id, week)
		return stats
	...
	def get_monthly_stats_for_player_of_year(cls, player_id, date_):
		...
		stats = dict()
		for month in months:
			stats[month.strftime("%Y-%m")] = cls.get_stats_for_player_of_month(
			    player_id, month.strftime("%Y-%m"))
		return stats

	@classmethod
	def get_yearly_stats_for_player(cls, player_id, date_):
		years = [
		    date(year, 1, 1) for year in range(date_.year - 4, date_.year + 1)
		]
		stats = dict()
		for year in years:
			stats[year.strftime("%Y")] = cls.get_stats_for_player_of_year(
			    player_id, year.strftime("%Y"))
		return stats
	...
	def get_weekly_stats_for_minigames_of_month(cls, date_):
		...
		stats = dict()
		for week in weeks:
			stats[week.strftime("%W")] = cls.get_stats_for_minigames_of_week(
			    week)
		return stats
	...
	def get_monthly_stats_for_minigames_of_year(cls, date_):
		...
		stats = dict()
		for month in months:
			stats[month.strftime(
			    "%Y-%m")] = cls.get_stats_for_minigames_of_month(month)
		return stats
	...
	def get_yearly_stats_for_minigames(cls, date_):
		...
		stats = dict()
		for year in years:
			stats[year.strftime("%Y")] = cls.get_stats_for_minigames_of_year(
			    year)
		return stats
	...
	def get_daily_stats_for_servers_of_month(cls, event, date_):
		...
		days = [
		    date(date_.year, date_.month, day)
		    for day in range(1, num_days + 1)
		]
		stats = dict()
		for day in days:
			stats[day.strftime("%Y-%m-%d")] = cls.get_servers_of_day(
	...
	def get_monthly_stats_for_servers_of_year(cls, event, date_):
		...
		stats = dict()
		for month in months:
			stats[month.strftime("%Y-%m")] = cls.get_servers_of_month(
			    event, month.strftime("%Y-%m"))
		return stats
	...
	def get_yearly_stats_for_servers(cls, event, date_):
		...
		stats = dict()
		for year in years:
			stats[year.strftime("%Y")] = cls.get_servers_of_year(
			    event, year.strftime("%Y"))
		return stats
	...
	def get_formatted_weekly_stats_of_player(cls, player_id):
		lists = [[
		    "Week", "Game", "W", "L", "D", "Total", "Unfinished", "Time"
		]]
		mg_weekly_stats = cls.get_weekly_stats_for_player_of_month(
		    player_id, date.today())
		weeks = [
		    date(date.today().year,
		         date.today().month, day) for day in range(1, 31, 7)
		]
		for week in weeks:
	...
	def get_formatted_monthly_stats_of_player(cls, player_id):
		lists = [[
		    "Month", "Game", "W", "L", "D", "Total", "Unfinished", "Time"
		]]
		mg_monthly_stats = cls.get_monthly_stats_for_player_of_year(
		    player_id, date.today())
	...
	def get_formatted_yearly_stats_of_player(cls, player_id):
		lists = [[
		    "Year", "Game", "W", "L", "D", "Total", "Unfinished", "Time"
		]]
		mg_yearly_stats = cls.get_yearly_stats_for_player(
		    player_id, date.today())
		years = [
		    date(year, 1, 1) for year in range(date.today().year - 4,
		                                       date.today().year + 1)
		]
		for year in years:
	...
	def get_formatted_stats_for_today_of_minigames(cls):
		lists = [[
		    "Game", "W", "L", "D", "Total", "AVG/M", "Unfinished", "Time"
		]]
	...
	def get_formatted_weekly_stats_of_minigames(cls):
		...
		mg_weekly_stats = cls.get_weekly_stats_for_minigames_of_month(
		    date.today())
		weeks = [
		    date(date.today().year,
		         date.today().month, day) for day in range(1, 31, 7)
	...
	def get_formatted_monthly_stats_of_minigames(cls):
		...
		mg_monthly_stats = cls.get_monthly_stats_for_minigames_of_year(
		    date.today())
		months = [date(date.today().year, month, 1) for month in range(1, 13)]
		for month in months:
			if mg_monthly_stats[month.strftime("%Y-%m")]:
