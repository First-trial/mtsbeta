...
from core.em import ALPHABET, STOP, NUMBERS
...
class QuizDiscord(SinglePlayerGame):
	def __init__(self, session):
		...
		self.categories = [
		    "General Knowledge", "Sports", "Films", "Music", "Video Games"
		]
	...
	async def start_game(self):
		...
		for i in range(1, len(self.categories) + 1):
			await MessageManager.add_reaction_and_event(
			    self.message, NUMBERS[i], self.player.id, self.choose_category,
			    NUMBERS[i])
	...
	async def choose_category(self, emoji):
		...
		for i in range(len(self.answers)):
			...
			await MessageManager.add_reaction_and_event(
			    self.message, emoji, self.player.id, self.choose_answer, emoji)
		await MessageManager.add_reaction_and_event(self.message, STOP,
		                                            self.player.id,
		                                            self.on_quit_game)
