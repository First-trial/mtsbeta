...
from core.em import ALPHABET, STOP
from core._hangman import Hangman, HANGMEN
		...
		for i in range(len(ascii_lowercase)):
			...
			if i < 13:
				await MessageManager.add_reaction_and_event(
				    self.message, emoji, self.player.id,
				    self.on_letter_reaction, emoji)
			if i >= 13:
				await MessageManager.add_reaction_and_event(
				    self.extra_message, emoji, self.player.id,
				    self.on_letter_reaction, emoji)
		await MessageManager.add_reaction_and_event(self.extra_message, STOP,
		                                            self.player.id,
		                                            self.on_quit_game)
