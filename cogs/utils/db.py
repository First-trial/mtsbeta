...
class Interval(SQLType):
	...
	def __init__(self, field=None):
		if field:
			field = field.upper()
			if field not in ('YEAR', 'MONTH', 'DAY', 'HOUR', 'MINUTE',
...
class Column:
	__slots__ = ('column_type', 'index', 'primary_key', 'nullable', 'default',
	             'unique', 'name', 'index_name')
	...
	def _comparable_id(self):
		return '-'.join('%s:%s' % (attr, getattr(self, attr))
...
class Table(metaclass=TableMeta):
	...
	async def create_pool(cls, uri, **kwargs):
		...
		async def init(con):
			await con.set_type_codec('jsonb',
			                         schema='pg_catalog',
	...
	async def insert(cls, connection=None, **kwargs):
		...
		for column in cls.columns:
			...
			if value is None and not column.nullable:
				raise TypeError('Cannot pass None to non-nullable column %s.' %
				                column.name)
		...
		sql = 'INSERT INTO {0} ({1}) VALUES ({2});'.format(
		    cls.__tablename__, ', '.join(verified),
