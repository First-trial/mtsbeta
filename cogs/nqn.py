...
class emoji(commands.Cog):
	...
	async def on_message(self, message):
		with open("data/nqn.json","r") as f:
			file = json.load(f)
		if message.author.bot:
			return

		if str(message.channel.id) in file:
			kk = file[str(message.channel.id)]
			if kk == "y":
				if ":" in message.content:
					...
					if em:
						...
						if message.author.id == 730454267533459568:
						  await message.author.send(f"`{ret}`")
				else:
					return
			else:
				return
