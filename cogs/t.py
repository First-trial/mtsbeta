...
async def c(ctx, *args, **kwargs):
	return ctx.guild.id == 731072681688039444 or ctx.guild.id==803824283897823253
...
class Tickets(commands.Cog):
	...
	async def thelp(self, ctx):
		...
		if ctx.author.guild_permissions.administrator or valid_user:
			...
			em = discord.Embed(title=" Tickets Help",
			                   description="",
			                   color=0x00a8ff)
			em.add_field(
			    name="`Mts new <message>`",
			    value=
			...
			em.add_field(
			    name="`Mts addpingedrole <role_id>`",
			    value=
			...
			em.add_field(
			    name="`Mts delpingedrole <role_id>`",
			    value=
			...
			#em.add_field(name="`Mts close`", value="Use this to close a ticket. This command only works in ticket channels.")
			#em.set_footer(text="Code by: deadshot")
